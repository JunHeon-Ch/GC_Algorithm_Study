...
public class Problem_16927 {
	...
	public static void ccw_rotation(int[][] A) {
		...
		for (int i=0; i<C; i++) {
            ...
            int left = i;
            int right = M-1-i;
            //for each cycle, excluding every value which doesn't vary during computation
            //각각의 사이클에 대하여 연산을 수행해도 제자리인 값을 제외시킨다.
            int rotation = R%(2*(right-left+1)+2*(bottom-top+1)-4);
